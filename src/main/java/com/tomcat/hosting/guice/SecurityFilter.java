...
public class SecurityFilter implements Filter {
	...
	public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain)
		...
		String pageName = getPageName(servletRequest);
		System.out.println("filter page " + pageName);
		if (pagesRequireAuthentication(pageName)
		&& false == isExcluded(req)
		&& false == securityCheck(req, response)) {
			System.out.println("failed authentication from " + pageName);
			response.sendRedirect(BaseServlet.getContextPath(req) + "index.xhtml");
		}
		else {
	...
	private boolean isExcluded(HttpServletRequest req) {
		...
		||  pageName.toLowerCase().endsWith(".woff2")
		||  pageName.toLowerCase().endsWith("login.xhtml")
		||  pageName.toLowerCase().endsWith("login")
		||  pageName.toLowerCase().endsWith("register")
		||  pageName.toLowerCase().endsWith("login.html")
		) return true;
		else return false;
	}
	private String getPageName(ServletRequest servletRequest) {
	...
	private boolean pagesRequireAuthentication(String pageName) {
		System.out.println("coming page:" + pageName);
		if (pageName.startsWith("userinfo")) return true;
		if (pageName.startsWith("examinfo")) return true;
		if (pageName.startsWith("examdetail")) return true;
		return false;
